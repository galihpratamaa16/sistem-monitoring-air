...
void saveJatuhTempo(int y, int m, int d, int h, int min) {
    dueYear = y; dueMonth = m; dueDay = d; dueHour = h; dueMinute = min;
    prefs.begin("keuangan", false);
    prefs.putInt("d_year", y); prefs.putInt("d_month", m); prefs.putInt("d_day", d);
    prefs.putInt("d_hour", h); prefs.putInt("d_min", min);
    prefs.end();
}
...
void handlePzem1(ModbusMessage msg, uint32_t token) {
    ...
    voltage1 = reg[0] / 10.0; current1 = reg[1] / 100.0; power1 = reg[2];
    energy1 = reg[3] / 100.0; frequency1 = reg[4] / 10.0; pf1 = reg[5] / 100.0;
}
void handleFlow(ModbusMessage msg, uint32_t token) {
    uint16_t reg;
    for(int i=0; i<JUMLAH_RUMAH; i++) {
        msg.get(3 + (i * 6), reg); flowRate[i] = reg / 100.0;
        msg.get(5 + (i * 6), reg); totalLiter[i] = reg / 100.0;
        msg.get(7 + (i * 6), reg); pulse[i] = reg;
...
void handleUltra(ModbusMessage msg, uint32_t token) {
    uint16_t reg;
    msg.get(3, reg); jarakAir = reg / 100.0;
    msg.get(5, reg); levelAir = reg / 1.0;
    msg.get(7, reg); nilaiTDS = reg;
    msg.get(9, reg); statusAir = reg;
}
void handleContactor1(ModbusMessage msg, uint32_t token) {
    uint16_t reg;
    msg.get(3, reg); modePompa1 = reg;
    msg.get(5, reg); statusPompa1 = reg;
}
void kirimModeContactor1(uint16_t mode) { MBContactor1.addRequest(millis(), 1, 6, 0, mode); }
...
void handleAuto() { modeMaster1 = 1; jadwalAktif = false; saveJadwal(); kirimModeContactor1(1); sendCORS(200, "text/plain", "OK"); }
void handleManual() { modeMaster1 = 0; jadwalAktif = false; saveJadwal(); kirimModeContactor1(0); sendCORS(200, "text/plain", "OK"); }
void handleOn() { if(modeMaster1 == 0) { relayMaster1 = 1; statusPompa1 = 1; kirimRelayContactor1(1); } sendCORS(200, "text/plain", "OK"); }
void handleOff() { if(modeMaster1 == 0) { relayMaster1 = 0; statusPompa1 = 0; kirimRelayContactor1(0); } sendCORS(200, "text/plain", "OK"); }
...
void handleSaveJadwal() {
    if (server.hasArg("mulai") && server.hasArg("selesai")) {
        jadwalMulai = server.arg("mulai"); jadwalSelesai = server.arg("selesai");
        jadwalAktif = true; saveJadwal(); modeMaster1 = 1; kirimModeContactor1(1);
    }
    sendCORS(200, "text/plain", "OK");
}
void handleDisableJadwal() { jadwalAktif = false; saveJadwal(); modeMaster1 = 1; kirimModeContactor1(1); sendCORS(200, "text/plain", "OK"); }
void handleSaveHarga() { if (server.hasArg("harga")) { tarifPerM3 = server.arg("harga").toFloat(); prefs.begin("harga", false); prefs.putFloat("tarif", tarifPerM3); prefs.end(); } sendCORS(200, "text/plain", "OK"); }
void handleSaveRekap() { if (server.hasArg("rekap")) saveRekapPemasukan(server.arg("rekap").toFloat()); sendCORS(200, "text/plain", "OK"); }
...
void handleResetPemasukan() { saveRekapPemasukan(0.0); sendCORS(200, "text/plain", "OK"); }
void handleResetPengeluaran() { totalPengeluaran = 0.0; riwayatPengeluaran = ""; prefs.begin("keuangan", false); prefs.putFloat("pengeluaran", 0.0); prefs.putString("riwayat", ""); prefs.end(); sendCORS(200, "text/plain", "OK"); }
void handleResetNeraca() { saveRekapPemasukan(0.0); handleResetPengeluaran(); sendCORS(200, "text/plain", "OK"); }
...
void setup() {
    ...
    loadJadwal(); loadHarga(); loadKeuangan();
    ...
    MBPzem1.onDataHandler(&handlePzem1); MBPzem1.connect();
    MBFlow.onDataHandler(&handleFlow); MBFlow.connect();
    MBUltra.onDataHandler(&handleUltra); MBUltra.connect();
    MBContactor1.onDataHandler(&handleContactor1); MBContactor1.connect();
    ...
    server.on("/auto", handleAuto); server.on("/manual", handleManual);
